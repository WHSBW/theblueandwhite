...
//
// Placement (all automatic, newest-first):
//   - Top Stories carousel : 5 most recent stories overall
//   - Hero lead + 2 side    : the 3 most recent overall
//   - Section columns       : latest 2 in Culture / Hot Spot / We Are Wharton
//   - Feature strip         : latest 1 in Editorial / Sports / Hot Spot
// The Wharton Brief video areas are left as-is (they're YouTube, set by hand).
...
(function () {
  ...
  function url(a) {
    var p = (a.github_path || '').replace(/^\/+/, '');
    return p ? '/' + p : '#';
  }
  function label(s) { return LABEL_CLASS[s] || 'label-news'; }
  function byline(a, withDate) {
    var parts = ['By <strong>' + esc(a.author_name || 'Staff') + '</strong>'];
    if (withDate && a.published_at) parts.push(esc(fmtDate(a.published_at)));
    return parts.join(' &nbsp;&middot;&nbsp; ');
  }
  function setHTML(id, html) { var el = document.getElementById(id); if (el) el.innerHTML = html; }

  // ---- slot generators (markup matches the existing homepage classes) ----
  function slideHTML(a) {
    return '<div class="carousel-slide">' +
      '<a href="' + url(a) + '">' +
        '<div class="slide-img"><div class="slide-badge">' + esc(a.section || 'Top Story') + '</div></div>' +
  ...
  function colEmptyHTML(section) {
    return '<div class="article-card"><p class="deck" style="color:var(--text-muted);font-size:12px;">More ' +
      esc(section) + ' stories coming soon.</p></div>';
  }
  function featureHTML(a, section) {
    if (!a) {
      return '<div class="feature-card" style="opacity:0.5;cursor:default;">' +
        '<span class="feature-label">' + esc(section) + '</span>' +
        '<div class="feature-headline" style="font-size:15px;">More ' + esc(section) + ' coming soon</div></div>';
    }
  ...
  // ---- Top Stories carousel (self-contained controller) ----
  var story = { idx: 0, total: 0, timer: null };
  function applyStory() {
    var track = document.getElementById('storyTrack');
    var dots = document.getElementById('storyDots');
    if (!track) return;
    track.style.transform = 'translateX(-' + (story.idx * 100) + '%)';
    if (dots) dots.querySelectorAll('.dot').forEach(function (d, i) {
      d.classList.toggle('active', i === story.idx);
    });
  }
  function autoplay() {
    clearInterval(story.timer);
    if (story.total > 1) {
      story.timer = setInterval(function () {
        story.idx = (story.idx + 1) % story.total;
        applyStory();
      }, 6000);
    }
  }
  window.BWHome = {
    moveStory: function (dir) {
      if (!story.total) return;
      story.idx = (story.idx + dir + story.total) % story.total;
      applyStory(); autoplay();
    },
  ...
  function buildCarousel(items) {
    ...
    if (!track) return;
    if (!items.length) {
      track.innerHTML = '<div class="carousel-slide"><div class="headline-main" style="color:var(--text-muted);">No stories published yet.</div></div>';
      if (dots) dots.innerHTML = '';
      story.total = 0;
      return;
    }
    track.innerHTML = items.map(slideHTML).join('');
    if (dots) dots.innerHTML = items.map(function (a, i) {
      return '<button class="dot' + (i === 0 ? ' active' : '') +
        '" onclick="BWHome.goStory(' + i + ')" aria-label="Story ' + (i + 1) + '"></button>';
    }).join('');
    story.idx = 0; story.total = items.length;
    applyStory(); autoplay();
  }
  ...
  async function populate() {
    ...
    [['colCulture', 'Culture'], ['colHotspot', 'Hot Spot'], ['colWharton', 'We Are Wharton']]
    ...
    if (fs) {
      fs.innerHTML = ['Editorial', 'Sports', 'Hot Spot'].map(function (s) {
        return featureHTML(bySection(s)[0], s);
      }).join('');
    }
  }
